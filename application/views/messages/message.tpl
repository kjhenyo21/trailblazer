...
-->
	<body onload="JavaScript:refresh(3000);">
		<div id="messages-ignored">
		<!-- Modal for Messages-->
			{if ($ignoredMessages)}
				{$noOfMessages = 0}
				{$row = 0}
				{foreach $ignoredMessages as $m}
		...
		<script>
			...
			function refresh(timeoutPeriod) {
				...
				$.ajax({
					type: "GET",
					url : 'messages/messages/checkNewMessages',
					dataType: "json",
					success: function(data){
						...
						if (data.latest_count != 0) {
							...
							$('#messages-ignored').load('messages/ignored_messages');
							$('body').append('<div id="messages-ignored"></div>');	
			...
			function confirmMessage(id, ref) {
				...
				$.ajax({
					type: "GET",
					url : 'messages/messages/confirmMessage?id=' + ref,
					success: function(data) {
						$('#messages').load('messages/load_messages');
					}
				});
			...
			function closeMessage(id, ref, no) {
				...
				$.ajax({
					type: "GET",
					url : 'messages/messages/ignoreMessage?id=' + ref,
					dataType: 'text',
					async: false,
					success: function(data) {
						console.log(data);
